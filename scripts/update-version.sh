...
###############################################################################
# Silver Assist ACF Clone Fields - Version Update Script
#
...
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    echo "Silver Assist ACF Clone Fields - Version Update Script"
    echo ""
    echo "Usage: $0 <new-version> [--no-confirm]"
    echo ""
    echo "Arguments:"
...
print_status "Updating Silver Assist ACF Clone Fields to version ${NEW_VERSION}"
...
# Get current version
CURRENT_VERSION=$(grep -o "Version: [0-9]\+\.[0-9]\+\.[0-9]\+" "${PROJECT_ROOT}/silver-assist-acf-clone-fields.php" | cut -d' ' -f2)
...
echo "  • Main plugin file: silver-assist-acf-clone-fields.php"
